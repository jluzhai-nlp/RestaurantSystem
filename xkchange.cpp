...
{
	time_break(); 
	ifstream read("KFC汉堡.txt");/*从txt中读取汉堡界面*/
...
{
	time_break(); 
	ifstream read("KFC小吃.txt");/*从txt中读取小吃界面*/
	char temp[1000]={};/*定义temp临时字符数组再将数据从txt中读取出来*/
	read.getline(temp,1000,0);
	cout<<temp<<endl;
	read.close();
...
{
	time_break(); 
	ifstream read("KFC饮料.txt");/*从txt中读取饮料界面*/ 
	char temp[1000]={};/*定义temp临时字符数组再将数据从txt中读取出来*/
	read.getline(temp,1000,0);
	cout<<temp<<endl;
...
void jiesu()/*增加*/ 
{
	ifstream read("点餐结束.txt");/*定义temp临时字符数组再将数据从txt中读取出来*/
	char temp[1000]={};
	read.getline(temp,1000,0);
	cout<<temp<<endl;
    read.close();
